...
def load_and_display(objid, duste=True, mod=None, mod_err=None, outfile=None):
    ...
    # Plot original observed spectrum if available
    """
    try:
        loaded_obs = fit_data['loaded_obs']
        wavelength = loaded_obs['wavelength']
        spectrum = loaded_obs['spectrum']
        no_emission = loaded_obs['no_emission']
        mask = loaded_obs['mask']
        wavelength = wavelength * 1e-4
        #spectrum *= factor
        
        print(np.median(spectrum[mask]))
        
        # Plot the NIRSpec spectrum
        ax.plot(wavelength, spectrum, '-', color='blue', alpha=0.8, lw=1.5, label='Observed spectroscopy')

    except KeyError:
        print("There original obs data are not stored here.")
...
def plot_sample_paper(zreds, logmasses, detections, data=None, color_scheme='plasma', save_path='z_mass_parameter_space.png'):
    """
    Plot z-M parameter space
